...
-- explain (analyze, buffers, verbose, format xml)
-- explain (analyze, buffers, verbose, format yaml)
...
;
-- 73 MB	57 MB: difference is 16 MB -> 16 bytes {difference of single row} * 1000000 (n of inserted rows)

-- There are some default column for each table so the previous optimization might not matter that much
...
* create a new table like another while also copying indexes, constraints, defaults:
s
```sql
...
-- NOTE:
-- When using a serial, and including defaults the new table will use the same sequence as the old because it simply copies the default expression.
-- When using an identity column (generated always as identity) and including identity the new table will use a different sequence.
-- therefore using serial is discouraged
```
...
* adding a new constraint on an existing table:

```sql
-- 'not valid' does not check the constraint against existing rows
alter table books add constraint fk_books_authors foreign key (author_id) references authors(id) not valid;
-- this validation only acquires 'SHARE UPDATE EXCLUSIVE' lock
...
* `\dt` list of tables
* `\dE` list of foreign tables
* `\di` list of indexes
* `\ds` list of sequences
* `\dv` list of views
* `\dm` list of materialized views
* The `-E` or `--echo-hidden` will display the actual query generated by backslash commands:
```bash
psql -d 'postgresql://root:my-secret-pw@localhost:5432/mydb?sslmode=disable' -E
\di+
# or equivalent:
> \set ECHO_HIDDEN true
```
* `\timing` to turn on timing on queries
* `\pset null 'Ø'` to print `Ø` for null values
* `\h {COMMAND}` to get more info on the command (ex: `\h SELECT`)

## Truncate

```sql
-- if B references A with a foreign key
truncate A; -- will return error
truncate A, B; -- ok
truncate A cascade; -- ok
```
